...
class ValueSlider(SocketWidget):
	def __init__(
		...
		# Input widget setup
		self._input_widget: QtWidgets.QSlider = QtWidgets.QSlider(QtCore.Qt.Horizontal)
		self._input_widget.setMinimum(-100)
		self._input_widget.setMaximum(100)
		self._input_widget.setValue(0)
		...
		# Listeners
		cast(QtCore.SignalInstance, self._input_widget.valueChanged).connect(self.editing_finished)
	...
	def input_data(self) -> list:
		...
		if len(result) == 0:
			result.append(ak.Array([self._input_widget.value()]))
		else:
	...
	def update_all(self):
		super().update_all()
		self._input_widget.setValue(int(self._prop_model.properties["Value"]))
		self.clearFocus()

	def editing_finished(self) -> None:
		self._prop_model.setData(self._prop_model.index(1, 1, QtCore.QModelIndex()), self._input_widget.value(), 2)
		self.clearFocus()
